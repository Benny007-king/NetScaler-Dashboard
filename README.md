...
- **Failover History & User Sessions**: date-range filters with a pop-up picker; **Excel/PDF** export (client-side). Sessions cover both **Gateway/AAA** deployments and **LB-only** appliances: it lists active **management (system) logins** (`/config/systemsession`) as well as VPN/AAA sessions, surfaces **failed login attempts** (status `Failed`, attempt time, source IP) and **locked accounts** (status `Locked`) — parsed from the appliance **audit log** for management accounts and from `aaauser` for Gateway ones — and resolves the **gateway name** from live ICA connections. `GET /api/session-debug` dumps per-node reachability/hostname plus the raw (redacted) NITRO session/user/audit fields to tune mapping across builds.
